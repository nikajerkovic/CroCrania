...
def preprocessDataAndPredict(feature_dict):
    ...
    if len(feature_dict) == 3 and all(key in feature_dict for key in alternative_columns):
        ...
        file = open("static/best_lr_model.pkl", "rb")
        scaled = True
        scaler =joblib.load('static/Standard_scaler_3.bin')
        ...
        file = open("static/best_svm_model_new.pkl", "rb")
        scaled = True
        scaler =joblib.load('static/Standard_scaler_18.bin')
        ...
        file = open("static/best_lda_model.pkl", "rb")
        scaled = False
    elif len(feature_dict) == 99:
        features = [float(value) for value in feature_dict.values()]
        file = open("static/lda_99.pkl", "rb")
        scaled = True
        scaler =joblib.load('static/Standard_scaler_99.bin')
